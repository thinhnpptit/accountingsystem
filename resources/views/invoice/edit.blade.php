        ...
        <div class="block-body">
            <form name="form" id="invoice" action="{{ route("invoices.update", $invoice->id)}}" method="post" class="form-horizontal">
                ...
                <div id="HTMLtoPDF">
                    ...
                    <!-- @foreach (\App\Sa::where([['parentid','=', $invoice->id],['from','=',3]])->get() as $item)
                        <p>    
                            {{$item}}
                        </p>
                    @endforeach
                    ...
                    @foreach (\App\Sa::where([['parentid','=', $invoice->id],['from','=',3]])->get() as $i => $item)
                    <div class="form-group row" id='line{{$i}}' >
                    <!-- {{\App\Subaccount::find($item->nameId)->accountname}} -->
                        <div class="col-sm-9">
                            <div class="row">
                                <div class="col-md-5">
                                    @if ($i==1)
                                    ...
                                    <select id="subvalue{{$i}}" name="subvalue{{$i}}" value="{{ old('subvalue' . $i )}}"
                                        ...
                                        <option value="{{ $subaccount->subid }}"
                                                {{ ($subaccount->accountname == \App\Subaccount::find($item->nameId)->accountname ) ? 'selected' : ''}}
